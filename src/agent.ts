...
export async function runChat(token: string, brandId: string, history: ChatMessage[]): Promise<string> {
  return withMcpClient(token, brandId, async (client) => {
    ...
    return "No pude terminar de procesar la consulta (demasiados pasos). Probá con una pregunta más acotada.";
  });
}
